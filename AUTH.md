...
1. Authorization
2. X-AgileSecurity-Signed-Headers
...
header `X-AgileSecurity-Request-Hash` that contains the SHA256 hash of the "canonical request" form of the "outer"
...
The second header, `X-AgileSecurity-Signed-Headers`, must contain a `;` separated list of headers that were included
when computing the outer request's hash. The list must include its self (`X-AgileSecurity-Signed-Headers`).

The `X-AgileSecurity-Signed-Headers` header is necessary because L7 load balancers and proxies (like ALB and API Gatway)
